...
export const createEcho = async (req, res) => {
    ...
    try {
        ...
        const now = new Date();
        const goLiveAt = goLiveLater === 'true' ? new Date(now.getTime() + 24 * 60 * 60 * 1000) : now;
...
export const updateEchoCaption = async (req, res) => {
    const { echoId } = req.params;
...
export const getMyEchos = async (req, res) => {
    try {
        const userId = req.user.id;
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 50;
